        ...
        IN_WALL: {
            ...
            ITEMS: {
                ...
                12: {
                    ...
                    _HEIGHT: 214,
                    _BOTTOM: 40,
                    UUID: '0E4BF9CF-135C-44E3-A77B-0FBFC7D599C7'
                }
            }
        },
        TOLERANCE: {
    ...
    (function (Items) {
        var InWall = (function () {
            ...
            InWall.prototype.compute = function (centerXY) {
                ...
                this.dir = line.dot(this.wall.getLine()) < 0 ? -1 : 1;
            };
            ...
            InWall.prototype.computeOffsetAndLength = function (activePoint, mousePos) {
                ...
                if (activePoint.equals(this.start)) {
                    var start = this.dir > 0 ? _line.start : _line.end;
                    var p = new IMAPIC2D.Line(start, this.end).closestPointOnLine(mousePos);
                    if (p.distanceTo(this.end) > minDis && p.distanceTo(start) > IMAPIC2D._DEFINES_.IN_WALL.MIN_DISTANCE_TO_WALL) {
                        this.start.copy(p);
                    }
                }
                else if (activePoint.equals(this.end)) {
                    var end = this.dir > 0 ? _line.end : _line.start;
                    var p = new IMAPIC2D.Line(this.start, end).closestPointOnLine(mousePos);
                    if (p.distanceTo(this.start) > minDis && p.distanceTo(end) > IMAPIC2D._DEFINES_.IN_WALL.MIN_DISTANCE_TO_WALL) {
                        this.end.copy(p);
                    }
                }
                else {
    ...
    (function (Items) {
        var Floorplan = (function () {
            ...
            Floorplan.prototype.checkWallItemPlacement = function (wall, inWallItem) {
                ...
                var InWallItemLength = inWallItem.getRawLength();
                var displacement = wall.getLine().scale(1.0);
                var start = wall.restrictLine.start.clone().add(displacement);
                for (var i = 0; i < items.length; i++) {
                    var item = items[i];
                    var dis = item.start.distanceTo(start);
                    if (dis < InWallItemLength) {
                        start.copy(item.end).add(displacement);
                    }
                    else {
                        isPlaceable = true;
                        break;
            ...
            Floorplan.prototype.newInWall = function (type, wall, scale, dir) {
                var _this = this;
                if (!wall) {
                ...
                var canPlace = false;
                var item = new Items.InWall(wall, type, scale, dir);
            ...
            Floorplan.prototype.loadFloorplan = function (floorplan) {
                ...
                floorplan['inWalls'].forEach(function (inwallJSON) {
                    ...
                    var inWall = _this.newInWall(inwallJSON['type'], wall, inwallJSON['lengthScaled'], inwallJSON['dir']);
                    if (inWall !== false) {
...
(function (IMAPIC2D) {
    var Engine = (function () {
        ...
        Engine.prototype.drawWallLengthLabels_Target = function (line) {
            ...
            if (this.toggle.isPC && input.css('display', 'none')) {
                input.show();
                input.focus();
            }
...
(function (IMAPIC3D) {
    var Engine = (function () {
        ...
        Engine.prototype.addModel = function(mesh,type){
            if(mesh.type !== 'Mesh')
                return;
        ...
        Engine.prototype.loadFrom2D = function (str) {
            ...
            // console.log(data);
              ...
              data['walls'].forEach((wall)=>{
                ...
                generator.generateWall(undefined,wall,_inWallsArray,scope.addModel.bind(scope));
              });

              // if(!('rooms' in data) ) {return;}
              data['rooms'].forEach((room)=>{
                var center = generator.generateRoom(room,scope.addModel.bind(scope));
