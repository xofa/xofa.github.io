        ...
        (function(Builds) {
            var Build = (function(_super) {
                ...
                Build.prototype.modifyMaterial = function(materialId, material) {
                    ...
                    var isWall = this.type == 'Wall';
                    this.traverse(function(child) {
                        if (child instanceof THREE.Mesh) {

                            // var repeat = child.material.map.repeat.clone();
                            // var width = child.material.map.image.width;
                            // var height = child.material.map.image.height;
                            ...
                            // child.material.repeat.set(repeat.x,repeat.y);
    ...
    (function(Models) {
        ...
        (function(Items) {
            var Item = (function(_super) {
                ...
                Item.prototype.raycast = function(element,p,dir){
                    var raycaster = new THREE.Raycaster();

                    var raydir = dir.normalize();
                    raycaster.set(p, raydir);
                ...
                Item.prototype.checkCollisionWall = function (box,dir) {

                    
                    var bSphere = box.getBoundingSphere();
                    var center = box.getCenter();
                    var size = box.getSize();


                    const walls = this._room.getWallItems();
                    for (var i = 0; i < walls.length; i++) {

                        var element = walls[i];

                        var posX = this.raycast(element,center,new THREE.Vector3(1,0,0));
                        var posZ = this.raycast(element,center,new THREE.Vector3(0,0,1));

                        var _box = element.boundingBox;
                        if (_box.intersectsSphere(bSphere)&&_box.intersectsBox(box)){

                            // var raycaster = new THREE.Raycaster();

                            // var raydir = new THREE.Vector3(dir.x,0,dir.z).normalize();
                            // raycaster.set(center, raydir);
                            // var intersects = raycaster.intersectObject(element);

                            // var castX = this.raycast(element,center,new THREE.Vector3(dir.x,0,0));
                            // var castZ = this.raycast(element,center,new THREE.Vector3(0,0,dir.z));
                            ...
                            // var moveX = castX&&Math.abs(dir.x) > castX.distance ? (castX.distance - size.x/2.0)*sX : dir.x;
                            // var moveZ = castZ&&Math.abs(dir.z) > castZ.distance ? (castZ.distance- size.z/2.0)*sZ : dir.z;

                            // console.log(moveX,moveZ);

                            // this.position.add(new THREE.Vector3(moveX,0,moveZ));
                            ...
                            //     var pos = p.clone().add(n.clone().multiply(size).divideScalar(2.0));
                    ...
                    //     this.helper = new THREE.Box3Helper(this.boundingBox);
                ...
                Item.prototype.checkSnapAndCollision1 = function(point,otherItems,snapDis,isOnwall) {

                    snapDis = snapDis!== undefined ? snapDis : 0;

                    var box = this.getBox();
                    var boxSize = box.getSize();

                    var dir = point.clone().sub(this.position);
                    box.translate(dir);
                    var center = box.getSize();


                    var boxSnap = box.clone();
                    if(Math.abs(dir.x) > Math.abs(dir.z)){
                        dir.x > 0 ? boxSnap.max.x += snapDis : boxSnap.min.x -= snapDis;
                    }else{
                        dir.z > 0 ? boxSnap.max.z += snapDis : boxSnap.min.z -= snapDis;
                    }

                    for (var i = 0; i < otherItems.length; i++) {
                       
                        var element = otherItems[i];
                        var _box = element.boundingBox === undefined ?  new THREE.Box3().setFromObject(element) : element.boundingBox;
                        ...
                        if(  _box.intersectsBox(boxSnap)) { //!_box.intersectsBox(box) && snapDis && !_box.intersectsBox(box) &&

                            var _boxSize = _box.getSize();
                            var _center = _box.getCenter();

                            var sizeHalf = boxSize.clone().add(_boxSize).multiplyScalar(0.5);
                            // sizeVec.x += 0.1;
                            // sizeVec.z += 0.1;

                            if(Math.abs(dir.x) > Math.abs(dir.z)){

                                this.position.x = _center.x;
                                this.position.x += dir.x > 0 ?  - sizeHalf.x : sizeHalf.x;
                            }else{
                                this.position.z = _center.z;
                                this.position.z += dir.z > 0 ?  - sizeHalf.z : sizeHalf.z;
                            }

                            if(isOnwall == true){

                                var hDis = box.max.y - _box.max.y;

                                if(Math.abs(hDis) < 40) {

                                    var centerToPositionDistance = this.position.y - center.y;
                                    this.position.y -= hDis  ;//+ centerToPositionDistance;
                                }
                            }
                ...
                Item.prototype.checkSnapAndCollision = function(box,dir,otherItems,snapDis,isOnwall) {

                    snapDis = snapDis!== undefined ? snapDis : 0;

                    var box = this.boundingBox.clone();
                    var boxSize = box.getSize();
                    var center = box.getCenter();
                ...
                Item.prototype.SnapAndCollision = function(point){

                    // var date = new Date();
                    // var t1 = /*oldDate.getSeconds() * 1000 +*/ oldDate.getMilliseconds();
                    ...
                    // console.time( 'collision Wall' );

                    var box = this.boundingBox === undefined ?  new THREE.Box3().setFromObject(this) : this.boundingBox.clone();
                    ...
                    var needCollision = this.name.indexOf('柜') < 0;//;

                    if( this.checkCollisionWall(box,dir)){
                        // console.timeEnd( 'collision Wall' );
                        return true;
                    }



                    // console.time( 'collision items' );
                    // if(needCollision){
                    //     const items = this._room.getItemsIf(this,needCollision );
                    //     if(this.checkSnapAndCollision(point,dir,items)){
                    //         console.timeEnd( 'collision items' );
                    //         return true;
                    //     };
                    // }
                    // console.timeEnd( 'collision items' );

                    // var needSnap = this.name.indexOf('柜') > -1 ;
                    // if(needSnap){
                    //     const items = this._room.getItemsIf(this,needSnap);
                    //     if(this.checkSnapAndCollision(point,dir,items,10,true)){return true;};
                    // }
                ...
                Item.prototype.move = function (point, collision) {

                    // console.time( 'intersect' );
                    if(collision!==false && this.SnapAndCollision(point)){
                ...
                Item.prototype.dragging = function(intersect,collision) {
                    return this.move(intersect.point, collision);
                };
...
(function(House3D) {
    ...
    (function(Houses) {
        var Room = (function() {
            ...
            Room.prototype.getMeshesIf = function(curitem,isTrue) {
                ...
                for (var key in this._items) {
                    ...
                    if (item.visible && (curitem!=item)&& isTrue )
                        this._items[key].getMeshes(meshes);
                }
            ...
            Room.prototype.getItemsIf = function(curitem,isTrue) {
            ...
            Room.prototype.getWallItems = function () {

                if(this.wallMeshes.length > 0) return this.wallMeshes;

                for (var key in this._builds) {
                    var build = this._builds[key];
                    if (build.type == "Wall"){
                        build.getMeshes(this.wallMeshes);
                        for (var i = 0; i < this.wallMeshes.length; i++) {
                            var element = this.wallMeshes[i];
                            // element.material.emissive.setHex(0xff0000);

                            if (element.boundingBox === undefined) {
                                element.geometry.computeVertexNormals();
                                var normalArray = element.geometry.attributes.normal.array;
                                var box = new THREE.Box3().setFromObject(element);

                                element.dir = new THREE.Vector3(normalArray[0], normalArray[1], normalArray[2]).normalize();

                                var dir = element.dir.clone().multiplyScalar(24).negate();
                                var tmp = box.getCenter().add(dir);
                                element.boundingBox = box.expandByPoint(tmp);

                                element.parent.add(new THREE.Box3Helper(element.boundingBox,0x00ff00));
                            }
...
(function(House3D) {
    ...
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    var UndoRedo;
    (function(UndoRedo) {
        var MoveItemCmd = (function(_super) {
            __extends(MoveItemCmd, _super);

            function MoveItemCmd(selectedObj, prePos, curPos) {
                var _this = _super.call(this) || this;
                _this.selectedObj = selectedObj;
                _this.prePos = prePos.clone();
                _this.curPos = curPos.clone();
                return _this;
            }
        ...
        UndoRedo.MoveItemCmd = MoveItemCmd;
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    ...
    (function(Houses) {
        var House = (function() {
            ...
            House.prototype.recomputeBoundingBox = function() {
                ...
                for (var key in items) {
                    ...
                    // item.parent.add(new THREE.Box3Helper(item.boundingBox));
                    // item.traverseVisible(function(mesh){
            ...
            House.prototype.dragStart = function(intersect) {
                ...
                if (obj.type.indexOf("Item") >= 0) {
                    ...
                    if (selectable === true) {
                        ...
                        this.prePos.copy(item.getWorldPosition());
                        if (this._combinationStart) {
                ...
                if (this._selectObject) {
                    if (this._selectObject.type.indexOf("Item") >= 0) {
                        ...
                        if (item.type == "OnFloorItem")
                            this._control.attach(item);
                        var curPos = item.getWorldPosition();
                        if (!this.prePos.equals(curPos)) {
                            var cmd = new House3D.UndoRedo.MoveItemCmd(item, this.prePos, curPos);
                            House3D.UndoRedo.Stack.getStack().exec(cmd);
                        }
                    }
                }
            };
            ...
            var House = (function(_super) {
                ...
                House.prototype.init_itemmenu = function(container) {
                    ...
                    var menuJSON = {
                        ...
                        detail: {
                            ...
                            src: "../static/img/xiangqing@3x.png",
                        }
                    };
                    var scope = this;
                    ...
                    function createMenuItem(item) {
                        ...
                        div.id = item.id;
                        div.src = item.src;
                        div.setAttribute("style", "backgroundColor:#666666;float:left;");
                        scope._itemMenu.appendChild(div);
                        return div;
                    }
                    createMenuItem(menuJSON.delete).addEventListener('click', function() { return _this.item_deleteEvent(); });
                    // createMenuItem(menuJSON.modifyColor).addEventListener('click', function() { return _this.item_modifycolorEvent(); });
                    // createMenuItem(menuJSON.modifyModel).addEventListener('click', function() { return _this.item_modifymodelEvent(); });
                    // createMenuItem(menuJSON.copyModel).addEventListener('click', function() { return _this.item_copymodelEvent(); });
                    // createMenuItem(menuJSON.separation).addEventListener('click', function() { return _this.item_separationEvent(); });
                    // createMenuItem(menuJSON.combination).addEventListener('click', function() { return _this.item_combinationEvent(); });
                    createMenuItem(menuJSON.detail).addEventListener('click', function() { return _this.item_detailEvent(); });
                };
                House.prototype.item_selectEvent = function() {
                    ...
                    if (this._selectObject && events && events.selectEvent) {
                        var item = this._selectObject;
                        events.selectEvent(item.getMetadata(),item);
                    }
                };
                House.prototype.item_deleteEvent = function() {
                    ...
                    if (this._selectObject && events && events.deleteEvent) {
                        var item = this._selectObject;
                        events.deleteEvent(item.getMetadata(),item);
                    }
                };
                ...
                House.prototype.init_buildmenu = function(container) {
                    ...
                    imgModifyColor.src = "../static/img/xiangqing@3x.png",
                ...
                House.prototype.build_modifycolorEvent = function() {
                    ...
                    if (this._selectObject && events && events.modifyColorEvent) {
                        ...
                        events.modifyColorEvent(build.getMetadata(),build.type);
                    }
                };
                    ...
                    if (obj.children !== undefined) {
                        obj.traverse(function(child) {
                            if (child instanceof THREE.Mesh) {
                                ...
                                newobj.material.side = THREE.DoubleSide;
                                item.add(newobj);
                            }
                        });
                    }
            ...
            var HouseSave = (function() {
                ...
                HouseSave.prototype.saveItem = function(house, houseDatas) {
                    ...
                    for (var _i = 0, items_5 = items; _i < items_5.length; _i++) {
                        var item = items_5[_i];
                        if (item.type == "Item") {} else if (item.type == "ItemGroup") {
                            var group = item;
...
(function(House3D) {
    var UndoRedo;
    (function(UndoRedo) {
        var AddItemCmd = (function(_super) {
            __extends(AddItemCmd, _super);

            function AddItemCmd(houseScene, item) {
                var _this = _super.call(this) || this;
                _this.houseScene = houseScene;
                _this.item = item;
                return _this;
            }
            AddItemCmd.prototype.undo = function() {
                this.houseScene.getModels().removeById(this.item.getMetadata().uuid);
                this.houseScene.getHouse().removeItem(this.item.getMetadata().uuid);
            };
            AddItemCmd.prototype.redo = function() {
                this.houseScene.getModels().addModel(this.item);
                this.houseScene.getHouse().addItem(this.item);
            };
            return AddItemCmd;
        }(UndoRedo.Command));
        UndoRedo.AddItemCmd = AddItemCmd;
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    var UndoRedo;
    (function(UndoRedo) {
        var RemoveItemCmd = (function(_super) {
            __extends(RemoveItemCmd, _super);

            function RemoveItemCmd(houseScene, item) {
                var _this = _super.call(this) || this;
                _this.houseScene = houseScene;
                _this.item = item;
                return _this;
            }
            RemoveItemCmd.prototype.undo = function() {
                this.houseScene.getModels().addModel(this.item);
                this.houseScene.getHouse().addItem(this.item);
            };
            RemoveItemCmd.prototype.redo = function() {
                this.houseScene.getModels().removeById(this.item.getMetadata().uuid);
                this.houseScene.getHouse().removeItem(this.item.getMetadata().uuid);
            };
            return RemoveItemCmd;
        }(UndoRedo.Command));
        UndoRedo.RemoveItemCmd = RemoveItemCmd;
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    var UndoRedo;
    (function(UndoRedo) {
        var ModifyModelCmd = (function(_super) {
            __extends(ModifyModelCmd, _super);

            function ModifyModelCmd(houseScene, item, newItem) {
                var _this = _super.call(this) || this;
                _this.houseScene = houseScene;
                _this.item = item;
            ...
            ModifyModelCmd.prototype.undo = function() {
                var newData = this.newItem.getMetadata();
                var newItem = this.houseScene.getHouse().getItem(newData.uuid);
                if (newItem) {
                    this.houseScene.getModels().removeById(newData.uuid);
                    this.houseScene.getHouse().removeItem(newData.uuid);
                    this.item.setMetadata(newData);
                    this.houseScene.getModels().addModel(this.item);
                    this.houseScene.getHouse().addItem(this.item);
                }
            };
            ModifyModelCmd.prototype.redo = function() {
                var data = this.item.getMetadata();
                var item = this.houseScene.getHouse().getItem(data.uuid);
                if (item) {
                    this.houseScene.getModels().removeById(data.uuid);
                    this.houseScene.getHouse().removeItem(data.uuid);
                    this.newItem.setMetadata(data);
                    this.houseScene.getModels().addModel(this.newItem);
                    this.houseScene.getHouse().addItem(this.newItem);
                }
            };
            return ModifyModelCmd;
        }(UndoRedo.Command));
        UndoRedo.ModifyModelCmd = ModifyModelCmd;
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    var UndoRedo;
    (function(UndoRedo) {
        var ModifyColorCmd = (function(_super) {
            __extends(ModifyColorCmd, _super);

            function ModifyColorCmd(build, preMaterialId, preMaterial, materialId, material) {
                var _this = _super.call(this) || this;
                _this.build = build;
                _this.preMaterialId = preMaterialId;
                _this.preMaterial = preMaterial;
                _this.materialId = materialId;
                _this.material = material;
                return _this;
            }
            ModifyColorCmd.prototype.undo = function() {
                if (this.build && this.preMaterial) {
                    this.build.modifyMaterial(this.preMaterialId, this.preMaterial);
                }
            };
            ModifyColorCmd.prototype.redo = function() {
                if (this.build && this.material) {
                    this.build.modifyMaterial(this.materialId, this.material);
                }
            };
            return ModifyColorCmd;
        }(UndoRedo.Command));
        UndoRedo.ModifyColorCmd = ModifyColorCmd;
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    var UndoRedo;
    (function(UndoRedo) {
        var CopyItemCmd = (function(_super) {
            __extends(CopyItemCmd, _super);

            function CopyItemCmd(houseScene, item) {
                return _super.call(this, houseScene, item) || this;
            }
            return CopyItemCmd;
        }(UndoRedo.AddItemCmd));
        UndoRedo.CopyItemCmd = CopyItemCmd;
    })(UndoRedo = House3D.UndoRedo || (House3D.UndoRedo = {}));
})(House3D || (House3D = {}));
var House3D;
(function(House3D) {
    ...
    (function(SceneEditor) {
        ...
        (function(Houses) {
            var HouseScene = (function() {
                ...
                HouseScene.prototype.addItem = function(item,flag) {
                    this._house.unDrag();
                    this._models.addModel(item);
                    this._house.addItem(item);
                    if(flag!=true){
                        var cmd = new House3D.UndoRedo.AddItemCmd(this, item);
                        House3D.UndoRedo.Stack.getStack().exec(cmd);
                    }
                };
                HouseScene.prototype.removeItem = function(data) {
                    this._house.unDrag();
                    var item = this._models.get(data.uuid);
                    if (item) {
                        var cmd = new House3D.UndoRedo.RemoveItemCmd(this, item);
                        House3D.UndoRedo.Stack.getStack().exec(cmd);
                    }
                ...
                HouseScene.prototype.getModels = function() {
                    return this._models;
                };
                ...
                HouseScene.prototype.itemModifyModel = function(data, newItem) {
                    this._house.unDrag();
                    var item = this._house.getItem(data.uuid);
                    if (item) {
                        this._models.removeById(data.uuid);
                        this._house.removeItem(data.uuid);
                        newItem.setMetadata(data);
                        this._models.addModel(newItem);
                        this._house.addItem(newItem);
                        var cmd = new House3D.UndoRedo.ModifyModelCmd(this, item, newItem);
                        House3D.UndoRedo.Stack.getStack().exec(cmd);
                    }
                };
                ...
                HouseScene.prototype.buildModifyColor = function(data, materialId, material) {
                    ...
                    if (build && material) {
                        var preMaterialId = data.materialId;
                        var preMaterial;
                        build.traverse(function(child) {
                            if (child instanceof THREE.Mesh && child.material) {
                                preMaterial = child.material.clone();
                                return;
                            }
                        });
                        build.modifyMaterial(materialId, material);
                        var cmd = new House3D.UndoRedo.ModifyColorCmd(build, preMaterialId, preMaterial, materialId, material);
                        House3D.UndoRedo.Stack.getStack().exec(cmd);
                    }
                };
                ...
                HouseScene.prototype.copyItem = function(metadata) {
                    ...
                    this._models.addModel(newItem);
                    this._house.addItem(newItem);
                    var cmd = new House3D.UndoRedo.CopyItemCmd(this, newItem);
                    House3D.UndoRedo.Stack.getStack().exec(cmd);
                };
                return HouseScene;
            }());
                ...
                DragItemControls.prototype.handMove = function(event) {
                    ...
                    if (intersects.length > 0) {
                        ...
                        if (newItem) {
                            ...
                            item.visible = true;
                            this._house.addItem(item);
                            if (item.dragging(intersect,false)) {
                                item.visible = true;
                                this._house.getHouse().cancelAddItem();
                            } else {
                                this._house.removeItem(item.getMetadata());
                            }
                        } else
                            item.dragging(intersect);
    ...
    (function(Controls) {
        var MoveControls = (function(_super) {
            ...
            function MoveControls(camera, domElement) {
                ...
                // 方太项目室内视角设置
                _this._yawObject.position.set(-400, 150, 0);
            ...
            MoveControls.prototype.getPitchObject = function() {
    ...
    (function(Controls) {
        var RockerControls = (function(_super) {
            ...
            RockerControls.prototype.init = function(container) {
                ...
                // imgrk.setAttribute("src", "../static/images/2.png");
                // imgrk.setAttribute("src", "../static/img/ft_spxx@2x.png");
            ...
            RockerControls.prototype.show = function() {
                this._domElement.setAttribute("style", "display:block;position:absolute; right:116px; bottom:40px;");
            };
    ...
    (function(Controls) {
        ...
        var LShape = (function(_super) {
            ...
            function LShape() {
                ...
                // 方太项目变动
                // var material = new GizmoMaterial({ color: 0xffff00,depthWrite:false}, 0x2B3423);
        ...
        var DShape = (function(_super) {
            ...
            function DShape() {
                ...
                // var material = new GizmoMaterial({ color: 0x4fc9fa }, 0xFFFF00);
                _this = _super.call(this, geometry, material) || this;
                return _this;
            }
            return DShape;
        ...
        var TransformGizmo = (function(_super) {
            ...
            TransformGizmo.prototype.scaleGeometry = function(box) {
                ...
                this.handleGizmos.SX = [
                    [new DShape(), [-x - 10, diff3, 0],
                        [0, Math.PI / 2, 0]
                    ],
                    [new DShape(), [x + 10, diff3, 0],
                        [0, Math.PI / 2, 0]
                    ],
                    [new VShape(), [-x - 10, diff4, 0],
                        [0, 0, 0]
                    ],
                    [new VShape(), [x + 10, diff4, 0],
                        [0, Math.PI, 0]
                ...
                this.handleGizmos.SZ = [
                    [new DShape(), [0, diff3, z + 10],
                        [0, -Math.PI, 0]
                    ],
                    [new DShape(), [0, diff3, -z - 10],
                        [0, 0, 0]
                    ],
                    [new VShape(), [0, diff4, z + 10],
                        [0, Math.PI / 2, 0]
                    ],
                    [new VShape(), [0, diff4, -z - 10],
                        [0, -Math.PI / 2, 0]
    ...
    (function(Controls) {
        var DragControls = (function(_super) {
            ...
            function DragControls(transform, domElement) {
                var _this = _super.call(this) || this;
                _this.prePos = new THREE.Vector3();
...
(function (House3D) {
    var Controls;
    (function (Controls) {
        var ChangeViewControls = (function (_super) {
            __extends(ChangeViewControls, _super);
            function ChangeViewControls(_editor) {
                var _this = _super.call(this) || this;
                _this.editor = _editor;
                return _this;
            }
            ChangeViewControls.prototype.handleDown = function () {
                if (this.preTouch == undefined) {
                    this.preTouch = new Date();
                }
                else {
                    var date = new Date();
                    var t1 = this.preTouch.getSeconds() * 1000 + this.preTouch.getMilliseconds();
                    var t2 = date.getSeconds() * 1000 + date.getMilliseconds();
                    if (Math.abs(t1 - t2) < 400) {
                        //this.editor.changeView();
                    }
                    this.preTouch = new Date();
                }
            };
            ChangeViewControls.prototype.onMouseDownEvent = function (event) {
            };
            ChangeViewControls.prototype.onTouchStartEvent = function (event) {
                if (event.touches.length !== 1) {
                    return;
                }
                this.handleDown();
            };
            return ChangeViewControls;
        }(Controls.EventControls));
        Controls.ChangeViewControls = ChangeViewControls;
    })(Controls = House3D.Controls || (House3D.Controls = {}));
})(House3D || (House3D = {}));
        ...
        (function(Environments) {
            var AmbientLight = (function(_super) {
                ...
                function AmbientLight() {
                    ...
                    // var hemiLight = new THREE.HemisphereLight(0xffffff, 0xffffff, 2.4);
        ...
        (function(Environments) {
            var Background = (function(_super) {
                ...
                Background.prototype.init = function() {
                    var skyGeo = new THREE.SphereBufferGeometry(2000, 200, 200);
    ...
    (function(SceneEditor) {
        var DesignEditor = (function() {
            ...
            DesignEditor.prototype.intiControls = function(container) {
                ...
                // this._changeViewControls = new House3D.Controls.ChangeViewControls(this);
                // this._eventMonitor.addEventListener(this._changeViewControls);
            ...
            DesignEditor.prototype.changeView = function () {
