...
(function () {
	...
	THREE.TransformControls = function (camera1, domElement) {
		...
		this.changeCamera = function (camera1) {
			camera = camera1;
		};
		...
		this.update = function () {
			...
			scale = worldPosition.distanceTo(camPosition) / 12 * scope.size;
			
			//this.position.copy(worldPosition);

			var box = new THREE.Box3();
			box.setFromObject(this.object);
			var pos = worldPosition.clone();
			pos.y = (box.min.y + box.max.y) / 2;
			...
			if (_mode === "translate") {
				...
				if (scope.space === "local") {
					...
					if (scope.axis === "XYZ") {
						...
						var max = 10;
						if (scope.axis === "X") {
							var scaleX = 1 + point.x / scope.scale.x;
							scaleX = scaleX > min ? scaleX : min;
							scaleX = scaleX < max ? scaleX : max;
							scope.object.scale.x = scaleX * oldScale.x;
						}
						if (scope.axis === "Y") {
							var scaleY = 1 + point.y / scope.scale.y;
							scaleY = scaleY > min ? scaleY : min;
							scaleY = scaleY < max ? scaleY : max;
							scope.object.scale.y = scaleY * oldScale.y;
						}
						if (scope.axis === "Z") {
							var scaleZ = 1 + point.z / scope.scale.z;
							scaleZ = scaleZ > min ? scaleZ : min;
							scaleZ = scaleZ < max ? scaleZ : max;
							scope.object.scale.z = scaleZ * oldScale.z;
						}
